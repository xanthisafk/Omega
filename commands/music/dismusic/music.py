...
class Music(commands.Cog, name="music"):
    ...
    async def connect_(self, ctx):
        ...
        try:
            ...
            return await ctx.send(f'{config.EMOTE_ERROR} Not connected to any voice channel')
        ...
        if player.is_connected:
            ...
            if player.channel_id == ctx.author.voice.channel.id:
                return await ctx.send(
                    f"{config.EMOTE_WARNING} Bot is already connected to your voice channel."
                )

            return await ctx.send(
                f"{config.EMOTE_ERROR} Bot is connected to a different voice channel. Can't join this."
            )

        channel = ctx.author.voice.channel
        self.bot.voice_users[ctx.author.id] = channel.id

        msg = await ctx.send(f"{config.EMOTE_WARNING} Connecting to **`{channel.name}`**")
        await player.connect(channel.id)
        player.bound_channel = ctx.channel
        await msg.edit(
            content=f"{config.EMOTE_OK} Connected to **`{channel.name}`** and bounded to {ctx.channel.mention}"
        )
    ...
    async def play_(self, ctx, *, query):
        ...
        if ctx.channel != player.bound_channel and player.bound_channel:
            return await ctx.send(
                f"{config.EMOTE_WARNING} Player is bounded to {player.bound_channel.mention}"
            )
        player.bound_channel = ctx.channel

        msg = await ctx.send(f"{config.EMOTE_WARNING} Searching for `{query}`")
        query = query.strip("<>")
        ...
        if not tracks:
            return await msg.edit(content=f"{config.EMOTE_ERROR} Could not find any song with that query.")
        ...
        if isinstance(tracks, wavelink.TrackPlaylist):
            ...
            msg.edit(
                content=f'{config.EMOTE_OK} Added the playlist **{tracks.data["playlistInfo"]["name"]}** with **{len(tracks.tracks)}** songs to the queue.'
            )
        else:
            await player.queue.put(tracks[0])

            await msg.edit(content=f"{config.EMOTE_OK} Added **{str(tracks[0])}** to the queue.")
    ...
    async def pause(self, ctx):
        ...
        if player.is_playing:
            if player.is_paused:
                return await ctx.send(f"{config.EMOTE_WARNING} Player is already paused.")

            await player.set_pause(pause=True)
            return await ctx.send(f"{config.EMOTE_WARNING} Player is now paused.")

        await ctx.send(f"{config.EMOTE_ERROR} Player is not playing anything.")
    ...
    async def resume(self, ctx):
        ...
        if player.is_playing:
            if not player.is_paused:
                return await ctx.send(f"{config.EMOTE_WARNING} Player is not paused.")

            await player.set_pause(pause=False)
            return await ctx.send(f"{config.EMOTE_WARNING} Player is now resumed.")
    ...
    async def seek(self, ctx, seconds: int, reverse: bool = False):
        ...
        if player.is_playing:
            if not player.is_paused:
                ...
                return await ctx.send(f"{config.EMOTE_OK} Seeked to {seconds} seconds.")
                #return await ctx.message.add_reaction(config.EMOTE_OK)

            return await ctx.send(
                f"{config.EMOTE_ERROR} Bot is paused. Resume the player to use this command."
            )
    ...
    async def volume(self, ctx, vol: int, forced=False):
        ...
        if vol < 0:
            return await ctx.send("{config.EMOTE_WARNING} Volume can't be less than 0")

        if vol > 100 and not forced:
            return await ctx.send("{config.EMOTE_WARNING} Volume can't greater than 100")
    ...
    async def loop(self, ctx, type: str = None):
        ...
        if type not in valid_types:
            return await ctx.send(f"{config.EMOTE_WARNING} Loop type must be `NONE`, `CURRENT` or `PLAYLIST`.")

        if len(player.queue._queue) < 1 and type == "PLAYLIST":
            return await ctx.send(
                f"{config.EMOTE_WARNING} There must be 2 songs in the queue in order to use the PLAYLIST loop"
            )

        if not player.is_playing:
            return await ctx.send(f"{config.EMOTE_ERROR} Player is not playing any track. Can't loop")

        player.loop = type

        await ctx.send(f"{config.EMOTE_OK} Player is now looping `{type}`")
    ...
    async def nowplaying(self, ctx):
        ...
        if not player.current:
            return await ctx.send(f"{config.EMOTE_ERROR} Nothing is playing.")
    ...
    async def equalizer(self, ctx):
        ...
        if ctx.channel != player.bound_channel:
            return await ctx.send(
                f"{self.bot} is bounded to {player.bound_channel.mention}"
            )
        ...
        while True:
            try:
                ...
                await msg.update(content=f'{config.EMOTE_WARNING} Message timed out.')
                break
