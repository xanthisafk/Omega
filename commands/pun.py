...
class Pun(commands.Cog):
    ...
    async def pun(self, ctx, id: str = None):
        ...
        try:
            if id == None:
                f = requests.get('https://icanhazdadjoke.com/', headers={
                                 "Accept": "application/json", "User-Agent": "My Library"}).json()
            else:
                url = 'https://icanhazdadjoke.com/j/' + id
                f = requests.get(url, headers={
                                 "Accept": "application/json", "User-Agent": "Personal discord bot: github.com/xanthisafk/coom"}).json()
